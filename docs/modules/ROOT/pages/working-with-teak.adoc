...
* `optOutFacebook` — deprecated; see xref:sdk5.adoc[SDK 5 Behaviors]. Don't pass a `facebookId` instead.
...
NOTE: Badge support depends on the device's launcher. The call returns `false` if the current launcher does not support badges. On Android, a badge is fundamentally set-or-not-set rather than numbered -- most launchers show only a dot regardless of the count you pass, though some show the number.
