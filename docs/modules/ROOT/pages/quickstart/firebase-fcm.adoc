...
----
+
TIP: Google occasionally bumps the plugin version. For the authoritative, current steps, see Firebase's own https://firebase.google.com/docs/android/setup[**Add Firebase to your Android project**, window=_blank] guide. You do **not** need to add the Firebase SDK dependencies it lists — `firebase-messaging` is covered as one of Teak's own required dependencies in the next step.
...
What you do need is Amazon push credentials: xref:ROOT:integrations:page$amazon-device-messaging.adoc[Finding your Amazon Device Messaging Credentials]. Store the resulting API key as `assets/api_key.txt` in your app module, per Amazon's https://developer.amazon.com/docs/adm/integrate-your-app.html#store-your-api-key-as-an-asset[Store Your API Key as an Asset, window=_blank] guide.
