...
Teak shows a small icon in the status bar and at the top of each notification. Setting it up has two parts: declaring the icon and tint-color resources, then generating the white-and-transparent icon assets that Android 5 and above require.
...
.res/values/teak.xml and res/values-v21/teak.xml
[source,xml]
----
...
NOTE: Replace `YOUR_ICON_FILE_NAME` with the name of your icon drawable (for example, `icon_white` in the steps below).
